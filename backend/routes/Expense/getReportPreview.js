...
router.get('/:config_id', async function(req, res, next) {
    ...
    try {
        ...
        for (var r = 0; r < contentResult.rows.length; r++) {
            ...
            // Build column_name -> value mapping for this version (ALL columns)
            var fullRowData = {};
            ...
            // Start with config columns in display order
            var rowData = {};
            for (var i = 0; i < configColumns.length; i++) {
            ...
            // Attach content row ID and full data for action logging
