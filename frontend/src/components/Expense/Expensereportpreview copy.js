                        ...
                        return (
                            ...
                            >
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    {cfg.action_id && (() => {
                                        var mapping = cfg.action_column_mapping || {};
                                        if (typeof mapping === 'string') { try { mapping = JSON.parse(mapping); } catch(e) { mapping = {}; } }
                                        return (
                                        <div style={{ fontSize: "11px", color: "#052049", marginTop: "4px", padding: "4px 8px", background: "#f0f5ff", borderRadius: "4px", display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                                            <span>{cfg.action_type === "send_email" ? "\u2709" : "\u2699"}</span>
                                            <span><strong>{cfg.action_name || "Action"}</strong></span>
                                            {cfg.action_button_label && <span>&middot; Button: "{cfg.action_button_label}"</span>}
                                            {mapping.email_to && <span>&middot; To: {mapping.email_to}</span>}
                                            {mapping.subject_template && <span>&middot; Subject: "{mapping.subject_template}"</span>}
                                        </div>
                                        );
                                    })()}
                                </div>
                                ...
                                {isSelected && (
                                    <button
                                        ...
                                        onClick={(e) => { e.stopPropagation(); this.handleDelete(cfg.id); }}
